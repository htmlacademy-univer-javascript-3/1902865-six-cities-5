...
export const userLogin = createAsyncThunk<void, TAuthData, DispatchStateExtra>(
  `${Actions.User}/login`,
  async ({ email, password }:{email:string, password:string}, { dispatch, extra: api }:{dispatch:AppDispatch, extra:AxiosInstance}) => {
...
export const userCheckAuth = createAsyncThunk<
  ...
  undefined,
  DispatchStateExtra
>(`${Actions.User}/login`, async (_arg, { dispatch, extra: api }:{dispatch:AppDispatch, extra:AxiosInstance}) => {
  try {
...
export const fetchOffers = createAsyncThunk<
  ...
  undefined,
  DispatchStateExtra
>(`${Actions.Offers}/fetch`, async (_arg, { dispatch, extra: api }:{dispatch:AppDispatch, extra:AxiosInstance}) => {
...
export const fetchOffer = createAsyncThunk<void, string, DispatchStateExtra>(
  `${Actions.Offer}/fetch`,
  async (id, { dispatch, extra: api }:{dispatch:AppDispatch, extra:AxiosInstance}) => {
    dispatch(setOfferLoadingStatus(LoadingStatus.Pending));

    const { status, data } = await api.get<TPlaceCardFull>(
      API_ROUTES.OFFERS.EXACT(id)
    );
...
export const fetchOffersNearby = createAsyncThunk<
  ...
  string,
  DispatchStateExtra
>(`${Actions.Offers}/nearby`, async (id, { dispatch, extra: api }:{dispatch:AppDispatch, extra:AxiosInstance}) => {
  dispatch(setOffersLoadingStatus(LoadingStatus.Pending));
  const { data: nearbyOffers } = await api.get<TPlaceCard[]>(
    API_ROUTES.OFFERS.NEARBY(id)
  );
...
export const fetchComments = createAsyncThunk<void, string, DispatchStateExtra>(
  ...
  async (id, { dispatch, extra: api }:{dispatch:AppDispatch, extra:AxiosInstance}) => {
    dispatch(setOffersLoadingStatus(LoadingStatus.Pending));
    const { data: comments } = await api.get<TReview[]>(
      API_ROUTES.COMMENTS.GET(id)
    );
...
export const createComment = createAsyncThunk<
  void,
  { form: TReviewFormState } & { offerId: string },
  DispatchStateExtra
>(
  `${Actions.Comments}/create`,
  async ({ offerId, form }, { dispatch, getState, extra: api }:{dispatch:AppDispatch, getState:()=>void, extra:AxiosInstance}) => {
    const { status } = await api.post<TReviewFormState>(
      API_ROUTES.COMMENTS.POST(offerId),
      form
    );
    ...
    if (
      ...
      state[Actions.Offer].offer?.id === offerId
    ) {
...
export const fetchFavorites = createAsyncThunk<
  ...
  undefined,
  DispatchStateExtra
>(`${Actions.Favorites}/fetch`, async (_arg, { dispatch, extra: api }:{dispatch:AppDispatch, extra:AxiosInstance}) => {
...
>(
  ...
  async ({ offerId, favoriteStatus }, { dispatch, extra: api }:{dispatch:AppDispatch, extra:AxiosInstance}) => {
    const { status } = await api.post<TReviewFormState>(
      API_ROUTES.FAVORITE.SET_STATUS(offerId, favoriteStatus ? 1 : 0)
    );
